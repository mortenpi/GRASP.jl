...
using AtomicLevels: AtomicLevels, Orbital, RelativisticOrbital, IntermediateTerm, degeneracy,
    κ2ℓ, κ2j
using HalfIntegers: HalfInteger, HalfInt

#
# Includes and imports related to compiled Fortran code.
# ------------------------------------------------------------------------------------------

let grasp_path_file = joinpath(@__DIR__, "../deps/grasp-path.jl")
    isfile(grasp_path_file) || error("deps/grasp-path.jl does not exist. Run `Pkg.build(\"GRASP\")`.")
    include(grasp_path_file)
end

const libgrasp_so = joinpath(@__DIR__, "..", "deps", "libgrasp.so")
